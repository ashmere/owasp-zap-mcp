...
def load_config():
    ...
    if LOG_LEVEL <= logging.DEBUG:
        ...
        env_vars = [
            "LOG_LEVEL", "ZAP_BASE_URL", "ZAP_API_KEY", 
            "SERVER_HOST", "SERVER_PORT", "ALLOWED_ORIGINS",
            "MCP_ALLOW_CREDENTIALS", "DOCKER_DEFAULT_PLATFORM"
        ]
        for var in env_vars:
    ...
    if not ZAP_BASE_URL.startswith(("http://", "https://")):
        warnings.append(f"ZAP_BASE_URL should start with http:// or https://, got: {ZAP_BASE_URL}")
