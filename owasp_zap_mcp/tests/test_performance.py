...
class TestConcurrentOperations:
    ...
    async def test_concurrent_health_checks(self, mock_zap_client_performance):
        ...
        assert (
            execution_time < 5.0
        ), f"Concurrent health checks took {execution_time:.2f}s"
    ...
    async def test_concurrent_spider_scans(self, mock_zap_client_performance):
        ...
        assert (
            execution_time < 10.0
        ), f"Concurrent spider scans took {execution_time:.2f}s"
    ...
    async def test_mixed_concurrent_operations(self, mock_zap_client_performance):
        ...
        assert (
            execution_time < 8.0
        ), f"Mixed concurrent operations took {execution_time:.2f}s"
...
class TestURLNormalizationPerformance:
    ...
    def test_url_normalization_batch_performance(self):
        ...
        assert (
            execution_time < 1.0
        ), f"URL normalization took {execution_time:.3f}s for 800 URLs"
    ...
    def test_url_normalization_edge_case_performance(self):
        ...
        assert (
            execution_time < 0.5
        ), f"Edge case normalization took {execution_time:.3f}s"
...
class TestMemoryUsageScenarios:
    ...
    async def test_large_alert_dataset_handling(self, mock_zap_client_large_data):
        ...
        assert (
            execution_time < 5.0
        ), f"Large alert processing took {execution_time:.2f}s"
    ...
    @pytest.mark.asyncio
    async def test_repeated_operations_memory_stability(
        self, mock_zap_client_large_data
    ):
...
class TestErrorRecoveryPerformance:
    ...
    async def test_error_recovery_performance(
        self, mock_zap_client_intermittent_errors
    ):
...
class TestLongRunningOperations:
    ...
    async def test_long_operation_performance(self, mock_zap_client_long_operations):
        ...
        assert (
            execution_time >= 2.0
        ), f"Long operation completed too quickly: {execution_time:.2f}s"
        assert (
            execution_time < 5.0
        ), f"Long operation took too long: {execution_time:.2f}s"
    ...
    async def test_concurrent_long_operations(self, mock_zap_client_long_operations):
        ...
        assert (
            execution_time >= 2.0
        ), f"Concurrent long operations too fast: {execution_time:.2f}s"
        assert (
            execution_time < 4.0
        ), f"Concurrent long operations too slow: {execution_time:.2f}s"
